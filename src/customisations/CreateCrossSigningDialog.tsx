...
import { SSOAuthEntry } from 'matrix-react-sdk/src/components/views/auth/InteractiveAuthEntryComponents';
...
// node_modules/matrix-react-sdk/src/components/views/dialogs/security/CreateCrossSigningDialog.tsx
...
export default class CreateCrossSigningDialog extends React.PureComponent<IProps, IState> {
    ...
    private async queryKeyUploadAuth(): Promise<void> {
        try {
            ...
            const canUploadKeysWithPasswordOnly = error.data.flows.some(f => {
                return f.stages.length === 1 && f.stages[0] === 'web3.signature';
            });
            this.setState({
    ...
    private doBootstrapUIAuth = async (makeRequest: (authData: any) => Promise<{}>): Promise<void> => {
        if (this.state.canUploadKeysWithPasswordOnly && this.state.accountPassword) {
            await makeRequest({
                type: /matrix.richtera.org/
                    .test(MatrixClientPeg.get().getHomeserverUrl()) ? 'web3.signature' : "m.login.password",
                identifier: {
                    type: 'm.id.user',
